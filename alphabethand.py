...
for  i in range(11,63):
        ...
        for j in range(1,56):
                ...
                with open('Hnd/binary'+str(i)+"-"+str(j)+".txt",'rb') as fp:
                          if fp is None:
                                  print("nullllllllll")
...
def main():
    ...
    mynet = network( [784,100,26] )
    mynet.stochasticGradientDescent( learnData, 20, 5 , 0.1, 0.001/len(itemlist[0]), \
                                   testData = validationData )        



# saving the current weight and biases
    mynet.save("MNIST-CrossEntropy-Network-Char")
